...
const pacientes = [
  ...
  {Id: 2, nome: "Joao", dataNascimento: '1962-09-16'},
...
module.exports.obterPaciente = async (event) => {
  try {
    ...
    if (!data.Item) {
      return {
        statusCode: 404,
        body: JSON.stringify({ error: "Paciente inexistente" }, null, 2),
      };
    }
...
module.exports.atualizarPaciente = async (event) => {
  ...
  try {
    ...
    if(error == 'ConditionalCheckFailedException') {
      ...
      message = `Recurso com ID ${pacienteId} inexistente e n pode ser atualizado`;
      statusCode = 404;
    }
...
module.exports.excluirPaciente = async (event) => {
  ...
  try{
    ...
    if(error == 'ConditionalCheckFailedException') {
      error = 'Paciente inexistente';
      message = `Recurso com ID ${pacienteId} inexistente e n pode ser excluído`;
      statusCode = 404;
    }
